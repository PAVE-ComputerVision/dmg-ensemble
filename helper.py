...
if __name__ == "__main__":
    ...
    output_dir = "".join(random.choices(string.ascii_letters + string.digits, k=10))
    
    for idx in range(int(args.chunks)):
        print(f"Process {idx}")
        if idx == int(args.chunks) - 1:
            subprocess.run([f"python3 dmg_ens_infer.py --time={args.time} --chunk_id={idx} --chunks={args.chunks} --output_dir={output_dir} "], shell=True)
        else:
            subprocess.Popen([f"python3 dmg_ens_infer.py --time={args.time} --chunk_id={idx} --chunks={args.chunks} --output_dir={output_dir} "], shell=True)
        time.sleep(1e-2)
#        if idx == int(args.chunks) - 1:
